...
<template>
  <div class="w-full bg-[#2626BF] py-[56px] lg:py-[126px] px-6 lg:px-24">
    <BaseContainer>
      <div
        class="w-full grid grid-cols-3 lg:grid-cols-6 gap-x-4 gap-y-4 lg:gap-y-[55px] items-center"
      >
        <h2
          class="lg:ps-4 col-span-3 text-center lg:text-start lg:col-span-2 text-[#ffffff] text-[32px] lg:text-[40px] font-black"
        >
          حواله ارز از همه جا
        </h2>
        <nuxt-link
        ...
        >
          ...
          <h3 class="flex flex-col gap-0.5 text-xs lg:text-sm">
            <span class="text-[#ffffff] text-nowrap">انتقال پول از/به</span>
            <span class="text-[#ffffff] font-black">{{ item.name }}</span>
          </h3>
        </nuxt-link>
      </div>
    </BaseContainer>
